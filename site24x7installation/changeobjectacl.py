...
This function is invoked by the event delivered by the SNS topic
...
TODO: Check for the command status to be finished 
TODO: Use SQS to queue the command for the timeout specified in SSM command
TODO: If the status is not completed, send to SQS
"""
import boto3
import logging
import os
...
# It takes a few seconds before the output is written to the bucket
time.sleep(10)
...
def s3_put_object_acl(bucket_name, object_key, client_s3):
    """ Put canned ACL 'bucket-owner-full-control' to provide the Master 
    account to have access to the log files generated by the SSM command

    Arguments:
    bucket_name - name of the bucket where the output is being stored
    object_key - object key for which we want to change the ACL (Note: the key 
    should have all the prefixes)
...
def fix_acl(value, client_s3):
    ...
    Arguments:
    value - A dict with the bucket and key details
