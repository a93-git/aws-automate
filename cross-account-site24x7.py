...
class Site24x7Installer():
    ...
    def s3_put_object_acl(self, bucket_name, object_key):
    ...
    def fix_acl(self):
        """ Loops through the _return_values list and attempts to fix the ACL
        ...
        for retval in self._return_values:
            _flow = False
            try:
                bucket_name = retval['Command']['OutputS3BucketName']
                command_id = retval['Command']['CommandId']
                prefix = retval['Command']['OutputS3KeyPrefix']
                instance_id = retval['Command']['Targets'][0]['Values'][0]
                script_name = retval['Command']['DocumentName']
                _flow = True
            except Exception as e:
            ...
            if _flow:
                if 'AWS-RunPowerShellScript' in script_name:
                    try:
                        ...
                        self.s3_put_object_acl(bucket_name, object_key_1)
                    except Exception as e:
                    ...
                    try:
                        ...
                        self.s3_put_object_acl(bucket_name, object_key_2)
                    except Exception as e:
                        logger.info("Error in putting ACL for {0}".format(object_key_2))
                        logger.error(e)
                elif 'AWS-RunShellScript':
                    try:
...
def lambda_handler(event, context):
    ...
    for region in REGIONS.values():
        try:
            a = Site24x7Installer(region, creds)
            a.install_agent()
            time.sleep(15)
            a.fix_acl()
        except Exception as e:
