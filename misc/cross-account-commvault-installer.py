...
Tag_Value - Value for the above tag key to look for
Output_S3_Bucket - Bucket to dump logs into
Output_S3_Key_Prefix_Linux - Folder to store the output from Linux VMs
Output_S3_Key_Prefix_Windows - Folder to store the output from Windows VMs
SNS_Topic - Topic to send notification to in the Master account
Role_ARN - ARN of the role to assume in the customer account
External_Id - String in the trust relationship
...
def lambda_handler(event, context):
    ...
    for region in REGIONS.values():
        try:
            try:
                a = CommvaultInstaller(region, creds)
                a.install_agent()
            except Exception as e:
